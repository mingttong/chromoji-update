    ...
    storage.get('type', function(data) {
        ...
        $('*:not(iframe):not(.emoji-inner):not(style):not(script):not(title):not(input):not(textarea)')
            ...
            .each(function() {
                ...
                content = twemoji.replace(textContent, '');
                if (content != textContent) {
                    ...
                    replacementNode.innerHTML = twemoji.parse(textContent)
                        .replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, (...args) => {
                            var src = args[1];
                            var style = `width:${fontSize};height:${fontSize};display:inline-block;background:url('${src}');background-size:100%;`;
                ...
                //     replacementNode.innerHTML = twemoji.parse(htmlEntities(self.textContent));
